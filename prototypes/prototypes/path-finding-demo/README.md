...
<!-- Quick link to setup instructions: [click here](#)  -->
...
7. https://mechwiz.github.io/Portfolio/jackal.html
8. https://gazebosim.org/docs/latest/sensors/

<!-- Taken Sarah (ideeën): -->
<!-- - Fix unidentified free cells being marked as unknown (-1): -->
<!--     - Higher Lidar samples or resolution -->
<!--     - Pre-process the occupancy grid from `/map` to fill in unknown cells (-1) that are likely to be free spaces based on the distance from the robot or that are surrounded by a certain amount of free cells -->
<!--     - Adjust certain settings in the costmap parameters that can decrease falsely identified unknown cells (-1), e.g. `observation_sources` or `observation_persistence` (probably more) -->
<!--     - Instead of selecting candidate boundaries based on whether the cell has only one unknown neighbouring cell (-1) select it only when it has multiple neighbouring unknown cells -->
<!-- - Use a costmap with custom settings (`inflation_layer`) to account for the robots size in the path finding algorithm -->
<!-- - Implement a more efficient search in place of Breadth First Search (BFS) -->
