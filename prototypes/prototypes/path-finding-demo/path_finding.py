...
class MapAnalyzer(Node):
    ...
    def publish_cmd_vel(self):
        ...
        # Test messages
        print("Current position: ", self.robot_position[0], self.robot_position[1])
        print("Curent goal:      ", x, y)
        ...
        if relative_distance < self.DISTANCE_THRESHOLD: # Stop when below threshold
            ...
            twist_msg.angular.z = 0.0 # Stop turning completely (or 0.1)
            twist_msg.linear.x = 0.4 # Could be increased? e.g. 0.4
            ...
            linear_speed = 0.4 * (1 - min(1.0, relative_distance / 1.0))  # Max speed at 0.5m away
    ...
    def map_callback(self, msg: OccupancyGrid):
        ...
        # Skip if a path is still being executed and re-evaluate at the half-way point
        if self.world_path is not None:
            # if len(self.world_path) > self.halfway_index:
            if len(self.world_path) > 1:
        ...
        self.halfway_index = len(self.world_path) // 2
