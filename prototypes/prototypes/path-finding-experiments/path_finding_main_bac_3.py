...
class MapAnalyzer(Node):
    ...
    def update_plot(self):
        ...
        # Plot all edge cells
        if self.edge_cells:
            y = [y for y, _ in self.edge_cells]
            x = [x for _, x in self.edge_cells]
    ...
    def breadth_first_search(self, grid, start: tuple):
        ...
        self.get_logger().info(f"edge_cells: {len(edge_cells)}")
    ...
    def map_callback(self, msg: OccupancyGrid):
        ...
        if self.world_path is not None and self.grid_path is not None:
            self.get_logger().info(f"world_path: {len(self.world_path)}")
            self.get_logger().info(f"grid_path: {len(self.grid_path)}")
