...
class FrontierExploration(Node):
    ...
    def frontier_exploration(self, occupancy_grid, map_origin, map_resolution):
        ...
        path = self.bfs(occupancy_grid, starting_position, map_origin, map_resolution)
    ...
    def publish_cmd_vel(self):
        ...
        neighbours = self.find_neighbouring_cells(self.map, column, row)
        values = [self.map[c, r] for (c, r) in neighbours]
    ...
    # NOTE: Old breadth first search algorithm (needs to be replaced with dijkstra)
    def bfs(self, occupancy_grid, starting_position, map_origin, map_resolution):

        # TEST
        start = time.time()

        frontier = Queue()
        frontier.put(starting_position)
        reached = {}
        reached[starting_position] = None
        ...
        while not frontier.empty():
            ...
            column_index, row_index = current
            neighbouring_cells = self.find_neighbouring_cells(occupancy_grid, column_index, row_index)

            for neighbour in neighbouring_cells:
                column_index, row_index = neighbour
                world_x, world_y = self.grid_world_coordinates(column_index, row_index, map_origin, map_resolution)
                distance = self.calc_relative_distance(world_x, world_y)

                if neighbour not in reached: 
                    # Exit the while-loop prematurely when a frontier cell has been found
                    if occupancy_grid[neighbour] == -1 and distance >= self.DISTANCE_TO_FRONTIER:
                        frontier_cell_found = True
                        break
                    # Avoid any obstacles using self.costmap
                    elif self.costmap[neighbour] == 0:
        ...
        # TEST
        elapsed = time.time() - start    
        self.get_logger().info(f'bfs took {elapsed:.3f}s')
